...
cases_sf <- cwi::town_sf %>%
  ...
  # sf::st_centroid() %>%
  left_join(town %>% filter(date == date_range[2]), by = "name") %>%
...
# finish wrangling this later
# case_change %>%
#   group_by(level, name) %>%
#   summarise(period = paste(days_ago, lag(days_ago), sep = "_"),
#             change = cases - lag(cases)) %>%
#   filter(!is.na(change)) %>%
#   mutate(pct_change = diff(change) / change[1]) %>%
#   filter(change >= t5_thresh) %>%
#   # extract(period, into = c("end", "start"), "(^t\\d{2})_(t\\d{2}$)", remove = FALSE)
#   mutate(range = str_match_all(period, "t\\d{2}")) %>%
#   # mutate(x = names(which(dates_elapsed == range[[1]])))
#   mutate(x = map(range, ~which(dates_elapsed == .[[1]])))
```
