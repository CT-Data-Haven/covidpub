...
```{r}
age_adj <- httr::GET("https://data.ct.gov/resource/7rne-efic.json", query = list("$where" = "ageadjustedcaserate_100k > 0")) %>%
  httr::content(as = "text", type = "application/json") %>%
  jsonlite::fromJSON() %>%
  select(date = dateupdated, race = hisp_race, ends_with("100k")) %>%
  mutate(date = as.Date(date)) %>%
  pivot_longer(-date:-race, names_to = c("type", "measure"), names_pattern = "(ageadjusted)?(.+)") %>%
  mutate_at(vars(race, type, measure), as_factor) %>%
  mutate(race = race %>%
           fct_relabel(str_remove, "NH ") %>%
           fct_recode(Latino = "Hispanic", AIAN = "American Indian or Alaskan Native", "Asian/Pac. Isl." = "Asian or Pacific Islander") %>%
           fct_relevel("White", "Black", "Latino"),
         type = fct_explicit_na(type, "crude") %>% 
           fct_recode("age-adjusted" = "ageadjusted") %>%
           fct_relabel(str_to_sentence),
         measure = fct_relabel(measure, str_replace, "(^\\w+)rate.+$", "\\1s") %>%
           fct_relabel(str_to_sentence),
...
case_by_race <- read_csv("input_data/covid_ct_race.csv") %>%
  mutate(race = as_factor(race) %>% 
           fct_relabel(str_remove, "NH ") %>% 
           fct_relabel(str_remove, " or Pacific Islander") %>% 
...
### Crude rate of detected cases per 10,000 residents by race/ethnicity, statewide as of `r race_date_fmt` {.viz-title}
...
### Crude rate of COVID-19-related deaths per 10,000 residents by race/ethnicity, statewide as of `r race_date_fmt` {.viz-title}
...
The longest continuous decrease in hospitalizations was between `r streak_fmt[1]` and `r streak_fmt[2]`, a streak of `r streak_dur` days. On the most recent day of data (`r format(hosp_recent$date, "%m/%d")`), the number of people hospitalized `r hosp_recent$direction` by `r abs(hosp_recent$change)` from the previous day.
