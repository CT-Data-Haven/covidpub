...
hhp <- list.files("input_data", pattern = "^hhp.+\\.csv", full.names = TRUE) %>%
  set_names(str_extract, "(?<=hhp_)(\\w*?)(?=\\d?_by_race)") %>%
  map(read_csv) %>%
  map(filter, name == "CT") %>%
  map(mutate, race = as_factor(race))
```

```{r}
housing_max <- round(max(hhp[["housing"]]$share), digits = 1)
housing_charts <- hhp[["housing"]] %>%
  mutate(tenure = as_factor(tenure)) %>%
  mutate(lbl = as_factor(tenure) %>%
           fct_recode("owned, paying mortgage" = "owned_mortgage") %>%
           fct_relabel(str_to_sentence)) %>%
  split(.$tenure) %>%
  imap(function(df, tenure) {
    lbl <- unique(df$lbl)
    billboarder(data = df, height = 350) %>%
      bb_barchart(mapping = bbaes(x = race, y = share, group = race), stack = TRUE, width = list(ratio = 0.5)) %>%
      bb_colors_manual(race_pal) %>%
      bb_legend(hide = TRUE) %>%
      bb_y_axis(label = list(text = "% housing insecure", position = "outer-top"),
                tick = list(format = htmlwidgets::JS("d3.format('0.0%')"),
                            values = seq(0, housing_max, by = 0.1)),
                max = housing_max) %>%
      bb_title(text = lbl)
  })
```
...
hhp[["housing"]] %>%
  ...
  bb_barchart(mapping = bbaes(x = race, y = share, group = lbl), stack = FALSE, width = list(ratio = 0.5)) %>%
  bb_colors_manual(c(Rented = pal[6], "Owned, paying mortgage" = pal[8]))
```
...
### Housing insecurity by tenure and race, 04/23/2020 to 05/26/2020 {.viz-title .minheight}

::: {.col-sm-6}

```{r out_housing_rent}
housing_charts[["rented"]]
```
