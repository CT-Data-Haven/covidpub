...
- Some values are given as rates and marked as such, where the number of cases is divided by population in order to meaningfully compare the relative magnitude of cases across areas.
...
## Greater New Haven {.section-title}

### Cases by location as of `r date_range_fmt[2]` {.viz-title}
...
gnh_trend %>%
  left_join(pops %>% select(-level), by = c("name")) %>%
  select(-level) %>%
  mutate(`Cases per 10k residents` = round(cases / total_pop * 1e4, digits = 1)) %>%
  arrange(-cases) %>%
  filter(date == date_range[2], cases >= reg_thresh) %>%
  select(-date, -total_pop) %>%
  rename_all(str_to_sentence) %>%
  kable(format = "html", align = "lrr", format.args = list(big.mark = ",")) %>%
  kable_styling(bootstrap_options = c("hover", "responsive")) %>%
  add_footnote(str_glue("Of local areas with at least {reg_thresh} cases"), notation = "none")
```

```{r}
exec_orders <- tribble(
  ~date, ~order,
  "2020-03-24", "Stay home order",
...
### Residents' average daily mobility in miles, `r format(mob_date_range[1], "%m/%d")` to `r format(mob_date_range[2], "%m/%d")` {.viz-title}
...
billboarder(data = mob) %>%
  bb_linechart(mapping = bbaes(x = date, y = m50_miles, group = name), width = 3) %>%
...
## Statewide {.section-title}

### Counts of confirmed COVID-19 cases by county, `r date_range_fmt[1]` to `r date_range_fmt[2]` {.viz-title}
...
### Confirmed cases by town, as of `r date_range_fmt[2]` {.viz-title}
...
color_scale <- colorBin(seq_pal, cases_sf$cases, bins = 5)
...
leaflet(cases_sf, width = 700, height = 500, 
  ...
  addCircleMarkers(data = sf::st_centroid(cases_sf),
                   radius = ~rad_scale(cases), color = seq_pal[5], 
                   weight = 1, opacity = 0.8, fillOpacity = 0.4,
                   label = ~str_glue("{name}: {cases}"),
                   labelOptions = labelOptions(className = "tooltip", direction = "top", offset = c(0, 5))) 
```



### Trends in statewide COVID-related testing, cases, hospitalizations, and deaths, `r date_range_fmt[1]` to `r date_range_fmt[2]` {.viz-title}
...
billboarder(data = test_x_cases, width = 700, height = 450) %>%
  bb_linechart(mapping = bbaes(x = date, y = value, group = measure), width = 3, type = "step") %>%
  bb_x_axis(label = list(text = NULL), type = "timeseries", 
            tick = list(format = "%m/%d", values = mondays)) %>%
...
### Change in number of confirmed new cases over 5-day periods {.viz-title}
...
case_change %>%
  ...
  kable_styling(bootstrap_options = c("hover", "responsive")) %>%
  add_footnote(str_glue("Of locations with at least {t5_thresh} new cases per 5-day period"), notation = "none")
```
...
* Data from Descartes Lab, "Mobility Changes in Response to COVID-19," "Mobility Changes in Response to COVID-19"
