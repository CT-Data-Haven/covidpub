...
datatable(counts_table, 
          ...
          rownames = FALSE, style = "bootstrap", class = "table table-striped", plugins = c("simple_incremental_bootstrap")) %>%
...
age_adj_race <- read_csv("input_data/ct_age_adj_rates.csv") %>%
  ...
  mutate(race = fct_reorder(race, cases_rate_10k, .desc = TRUE) %>% fct_relevel("Total")) %>%
  arrange(race)
...
weekly_tests <- test_x_cases %>%
  filter(measure == "Individuals tested") %>%
  mutate(monday = floor_date(date, unit = "weeks", week_start = 1),
         daily_tests = value - lag(value, default = first(value))) %>%
  group_by(name, monday) %>%
...
```{r tests_per_wk, fig.width=7, fig.height=4.5}
